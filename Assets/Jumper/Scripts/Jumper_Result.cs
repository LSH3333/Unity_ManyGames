...
public class Jumper_Result : MonoBehaviour
{
    ...
    {
        // best score update
        ManageApp.singleton.updateBestScore((int)JumperManagerGame.singleton.Score);
        ...
        for(int i = 0; i < 10; i++)
        {
            ...
            if((int)JumperManagerGame.singleton.Score > (int) list_score[i])
            {
                list_name.Insert(i, ManageApp.singleton.NickName);
                list_score.Insert(i, (int)JumperManagerGame.singleton.Score);
    ...
    public void SendPlayerDataToNCMB()
    {
        ...
        obj.Add("Score", JumperManagerGame.singleton.Score);
